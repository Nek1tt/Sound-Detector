...
async def process_sound(message: types.Message, state: FSMContext):
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        try:
            files = {
    'file': (f'{sound_name}.ogg', file_bytes, 'audio/ogg')
}
...
async def stream_logs_handler(message: types.Message):
    ...
    async with httpx.AsyncClient(headers=HEADERS, timeout=10.0) as client:
        await client.post(f"{API_URL}/start_stream", json={"chat_id": user_id})
        while streaming_active.get(user_id):
            if not streaming_active.get(user_id):
                break
...
async def stop_stream(message: types.Message):
    user_id = message.from_user.id
    streaming_active[user_id] = False

    async with httpx.AsyncClient(timeout=10.0) as client:
        try:
            await client.post(f"{API_URL}/stop_stream", json={"chat_id": user_id}, headers=HEADERS)
        except Exception as e:
            print(f"Ошибка стопа: {e}")
...
async def stop_stream(message: types.Message):
    ...
    async with httpx.AsyncClient() as client:
        await client.post(f"{API_URL}/stop_stream", json={"chat_id": user_id})
